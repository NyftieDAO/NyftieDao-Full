...
function UserNFTs({userId}) {
  ...
  async function getOfferings(){
    ...
    if (isInitialized) {
      const options = { chain: 'ropsten', address: userId};
  ...
  function generateOfferingDisplay(title, nftURL, owner, index, blockNumber, imageURL, tokenID){
    console.log(title);
                            ...
                            `<div key=${index} class="d-item col-lg-3 col-md-6 col-sm-6 col-xs-12">
                              <div class="nft__item">
                                  ...
                                  <div class="nft__item_info">
                                    <h4 style="width: 200px;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;">${title}</h4> 
                                    <h4>${title}</h4>           
                                  </div>
                                  <a class="btn-main" href="/Listing/${owner}/${tokenID}">Sell</a>
                              </div>
                          </div>`
